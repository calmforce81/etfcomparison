...
export default function CompareTable({ theme }: CompareTableProps) {
  const k = theme.k;
  const t = theme.t;
  ...
  const rows = [
    ...
    {
      label: '일간 등락',
      kv: fmtChange(k.change),
      tv: fmtChange(t.change),
      winner: k.change > t.change ? 'k' : 't',
      upIsGood: true,
    },
    {
      label: '1개월 수익률',
    ...
    {
      label: 'NAV 괴리율',
      kv: (k.discount > 0 ? '+' : '') + k.discount + '%',
      tv: (t.discount > 0 ? '+' : '') + t.discount + '%',
      winner: null,
    },
  ];
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.label} className={styles.row}>
              ...
              <td className={`${styles.rowVal} ${row.winner === 'k' ? styles.winK : ''}`}>
                {row.kv}
              </td>
              <td className={`${styles.rowVal} ${row.winner === 't' ? styles.winT : ''}`}>
                {row.tv}
              </td>
              <td className={styles.rowWinner}>
                ...
                {!row.winner && <span className={styles.dash}>—</span>}
              </td>
            </tr>
          ))}
        </tbody>
