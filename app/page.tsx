...
export default function Home() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      // API 응답에 meta 없으면 Mock meta 보강
      const enriched: ThemeData[] = (tData.themes as ThemeData[]).map((th, i) => ({
        ...th,
        fetchedAt: tData.updatedAt ?? new Date().toISOString(),
        k: { ...th.k, isin: th.k.isin ?? THEMES[i].k.isin, exchange: th.k.exchange ?? 'KRX', meta: th.k.meta ?? THEMES[i].k.meta },
        t: { ...th.t, isin: th.t.isin ?? THEMES[i].t.isin, exchange: th.t.exchange ?? 'KRX', meta: th.t.meta ?? THEMES[i].t.meta },
      }));
      setThemes(enriched);
        ...
        <div className={styles.inner}>
          ...
          {activeTab === 'dashboard' && (
            <div className={styles.dashboard}>
              {/* 조회시간 + 신뢰도 패널 */}
              <DataTrustPanel theme={theme} />

              <div className={styles.kpiGrid}>
                <KpiCard label="현재가"
                  kVal={k.price.toLocaleString()+'원'} tVal={t.price.toLocaleString()+'원'}
                  kChange={fmtPct(k.change)} tChange={fmtPct(t.change)}
                  kUp={k.change>=0} tUp={t.change>=0} icon="₩" />
                <KpiCard label="1개월 수익률"
                  kVal={fmtPct(kRet1m)} tVal={fmtPct(tRet1m)}
                  winner={kRet1m>=tRet1m?'k':'t'}
                  kUp={kRet1m>=0} tUp={tRet1m>=0} icon="📈" />
                <KpiCard label="순자산 (AUM)"
                  kVal={k.aum} tVal={t.aum}
                  winner={k.aumRaw>=t.aumRaw?'k':'t'} icon="💰" />
                <KpiCard label="총보수 / 괴리율"
                  kVal={k.expense+'%'} tVal={t.expense+'%'}
                  kChange={'괴리 '+(k.discount>=0?'+':'')+k.discount+'%'}
                  tChange={'괴리 '+(t.discount>=0?'+':'')+t.discount+'%'}
                  winner={k.expense<=t.expense?'k':'t'} icon="⚖️" />
              </div>

              <div className={styles.chartRow}>
                <div className={styles.chartMain}><ChartCard theme={theme} /></div>
                <div className={styles.chartSide}>
                  <DonutCard kAum={k.aumRaw} tAum={t.aumRaw} kLabel={k.aum} tLabel={t.aum} />
                  <div className={styles.volSummary}>
                    ...
                    <div className={styles.vsBar}>
                      <div className={styles.vsBarK} style={{ width: (k.volRaw/(k.volRaw+t.volRaw)*100)+'%' }}/>
                    </div>
                  </div>
                </div>
              </div>
